    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        if "basic_cleaning" in steps_to_execute:
            ...
            mlflow.run(
                ...
                entry_point="main",
                parameters={
                    "input_artifact": "sample.csv:latest",import os
import mlflow
import tempfile
import json
import hydra
...
def go(config: DictConfig):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        if "download" in steps_to_execute:
            logger.info("Running 'download' step")
            mlflow.run(
                uri=f"{config['main']['components_repository']}#get_data",  # Corrected for Git-compatible syntax
                entry_point="main",
                parameters={
                    "sample": config["etl"]["sample"],
                    "artifact_name": "sample.csv",
        ...
        if "data_split" in steps_to_execute:
            ...
            mlflow.run(
                uri=f"{config['main']['components_repository']}#train_val_test_split",  # Corrected for Git-compatible syntax
                entry_point="main",
                parameters={
